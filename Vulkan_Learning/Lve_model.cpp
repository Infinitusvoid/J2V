...
namespace std {
    ...
    struct hash<lve::Lve_model::Vertex> {
        size_t operator()(lve::Lve_model::Vertex const& vertex) const {
            size_t seed = 0;
...
namespace lve {
    ...
    Lve_model::Lve_model(LveDevice& device, const Lve_model::Builder& builder) : lveDevice{ device } {
    ...
    std::unique_ptr<Lve_model> Lve_model::createModelFromFile(
    ...
    void Lve_model::Builder::loadModel(const std::string& filepath) {
        ...
        if (!tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, filepath.c_str())) {
            std::cout << " filepath.c_str()" << filepath.c_str() << "\n";
